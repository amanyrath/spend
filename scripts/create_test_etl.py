"""Test ETL script for generating 30-user test set.

This script generates a 30-user test set with:
- 3 users per persona (15 total constructed users)
- 15 additional users with full transaction history (unconstructed)
- All users get full transaction history

Based on run_etl.py but simplified for test set generation.
"""
...
def get_firebase_stats(timeout_seconds: int = 10) -> Dict[str, int]:
    ...
    client = get_db()
    if client is None:
        print("  Firebase not available (not initialized)")
        return stats
    ...
    try:
        ...
        try:
            ...
            for doc in users_ref.limit(500).stream():
                ...
                if time.time() - start_time > timeout_seconds:
                    print(f"  Timeout reached, stopping at {len(users_list)} users")
                    break
        ...
        if not users_list:
            print("  No users found, skipping subcollection counts")
        else:
            ...
            for i, user_doc in enumerate(users_list[:max_users]):
                # Check timeout
                if time.time() - start_time > timeout_seconds:
                    print(f"  Timeout reached, stopping subcollection counts at user {i}/{max_users}")
                    break
                ...
                try:
                    ...
                    print(f"    Error counting subcollections for user {user_doc.id}: {e}")
                    continue
            
            if max_users < len(users_list):
                print(f"  Note: Only counted subcollections for {max_users}/{len(users_list)} users (sample)")
        ...
        print(f"  Warning: Could not get Firebase stats: {e}")
...
def generate_and_load_data(days: int, output_dir: str, 
                           wipe_sqlite: bool, db_path: Optional[str] = None) -> bool:
    """Generate synthetic test data and load into SQLite.
    
    Generates exactly 30 users with 3 users per persona (15 total constructed)
    and 15 unconstructed users with full transaction history.
    ...
    try:
        ...
        print("=" * 60)
        print(f"Users: 30 (3 per persona + 15 unconstructed)")
        print(f"Days: {days}")
        print(f"Output directory: {output_dir}")
        print("=" * 60)
        
        # Generate 30 users with 3 per persona
        generate_all_data(count=30, output_dir=output_dir, days=days, users_per_persona=3)
...
def main():
    parser = argparse.ArgumentParser(
        description="Test ETL script for 30-user test set with 3 users per persona",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Full pipeline with prompts
  python scripts/create_test_etl.py --wipe-sqlite --push-to-firebase
  
  # Archive before wiping
  python scripts/create_test_etl.py --wipe-sqlite --archive-before-wipe --push-to-firebase
  
  # Generate and process only (no push)
  python scripts/create_test_etl.py --wipe-sqlite
  
  # Custom days
  python scripts/create_test_etl.py --days 90 --time-windows 30d
  
  # Skip processing (just generate and load)
  python scripts/create_test_etl.py --wipe-sqlite --skip-processing

This script generates exactly 30 users:
  - 3 users per persona (15 total constructed)
  - 15 additional users with full transaction history (unconstructed)
        """
    )
    ...
    print("=" * 60)
    print("SpendSense Test ETL Pipeline (30-user test set)")
    print("=" * 60)
    ...
    if not generate_and_load_data(args.days, args.output_dir, 
    ...
    print("\n" + "=" * 60)
    print("Test ETL Pipeline Complete!")
    print("=" * 60)
